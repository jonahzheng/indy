...
begin
  case AIPVersion of
    Id_IPv4: begin
      with TIdIn4Addr(AInAddr).S_un_b do begin
      //masked with FF to prevent overflow problems on FPC
        s_b1 := Sys.StrToInt(Fetch(AIP, '.')) and $FF;    {Do not Localize}
        s_b2 := Sys.StrToInt(Fetch(AIP, '.')) and $FF;    {Do not Localize}
        s_b3 := Sys.StrToInt(Fetch(AIP, '.')) and $FF;    {Do not Localize}
        s_b4 := Sys.StrToInt(Fetch(AIP, '.')) and $FF;    {Do not Localize}
      end;
    end;
    Id_IPv6: begin
